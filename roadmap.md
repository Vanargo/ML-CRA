...
## 49. ST08_14 — контракт первого выпуска 0.1.0

John принял и закрыл ST08_13C и отдельно авторизовал
`ST08_14_release_0_1_0_scope_channel_and_execution_contract_design`. Этот блок
проектирует выпуск, но не меняет development-version — версию разработки —
`0.1.0.dev0`, не создаёт тег, GitHub Release, TestPyPI/PyPI-публикацию и не
изменяет научное состояние.
...
Принятая релизная граница сохраняет утверждённую последовательность: первым
поддерживаемым выпуском является `0.1.0` с тегом `v0.1.0`, а `1.0.0`
откладывается до выпуска `0.1.0`, анализа последующих замечаний и отдельного
решения John о стабильности публичного API. Основной будущий канал — GitHub
Release. PyPI и TestPyPI остаются `deferred_not_authorized` — отложенными и не
разрешёнными.

MAPE-K-анализ и взвешенная матрица сравнили прямой `1.0.0`, объединённый
немедленный `0.1.0` и поэтапный путь. По шкале 1–5 получены соответственно
`1.40`, `2.60` и `5.00`; выбран третий вариант. Его этапы: текущий контракт,
отдельная финализация кандидата, отдельный повторный аудит точного кандидата,
отдельно разрешённое выполнение неизменяемого GitHub Release и только при
будущем самостоятельном решении — публикация PyPI через Trusted Publishing.

Будущий GitHub Release должен создаваться как draft — черновик — с заранее
приложенными `sdist`, `wheel` и `SHA256SUMS`, после чего публиковаться как
immutable release — неизменяемый выпуск. Хеши и attestations — подтверждения
происхождения — удостоверяют целостность и provenance — происхождение, но не
заменяют тестирование, научную валидацию или решение John о риске.

Новый манифест ST08_14 является преемником текущего дерева. ST08_13C сохранён
как неизменяемая история; его проектный `PASS` не переинтерпретирован как
разрешение выпуска. Контракт, 14 мутационных проверок, working/published
baseline, source/documentation/secret assurance и 18/18 защищённых научных
артефактов должны пройти до технического закрытия ST08_14.

## 50. Предлагаемый следующий блок
...
ST08_14A должен заменить `0.1.0.dev0` на финальную `0.1.0`, согласовать
метаданные и двуязычные release notes — примечания к выпуску — и собрать
воспроизводимые локальные артефакты без тега или внешней публикации. Затем
потребуется отдельный аудит ST08_14B; выполнение GitHub Release относится
только к ещё более позднему ST08_14C.

## 51. Задачи John

1. Принять либо вернуть только контракт ST08_14.
2. При принятии присвоить `ACCEPTED_BY_JOHN` и `TASK_CLOSED`.
