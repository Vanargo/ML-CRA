...
## 76. ST08_15 — анализ после выпуска 0.1.0 и корректирующий контракт 0.1.1

John принял и закрыл ST08_14C, разрешил merge — слияние — PR №17 и post-merge
`assurance`, а затем отдельно авторизовал
`ST08_15_v0_1_0_post_release_review_and_v0_1_1_corrective_contract_design`.
Наблюдённый post-merge run `35184447363`, job `105083380082` имеет conclusion
`success` и `final-evidence=PASS`. Профиль `CHANGE` текущего блока ограничен
контрактом и проверками: исходный код продукта, версия `0.1.0`, release assets —
активы выпуска, PyPI/TestPyPI и научные артефакты защищены от изменения.
...
### 76.1. Наблюдение и классификация дефекта

Read-only — только для чтения — проверка GitHub 17 сентября 2026 года
подтвердила единственный release `v0.1.0`, его immutable-state — неизменяемость,
точный commit и неизменные SHA-256 трёх активов. Принятая evidence ST08_14C и
live observation — текущее наблюдение — согласуются.

В `v0.1.0` документирован остаточный дефект: README и doctor сохраняют
предрелизное утверждение `release_ready=false` с
`John_release_authorization_not_granted`. Среда и ограниченные научные
результаты от этого не меняются. Перезапись выпущенных байтов запрещена;
исправление может существовать только в новой версии.

### 76.2. Формальный выбор и семантика коррекции

Взвешенная сумма по шести критериям — целостность неизменяемого выпуска,
пользовательская согласованность, версия/совместимость, управление полномочиями,
цепочка поставки и стоимость — дала `2.50`, `1.70`, `3.95` и `4.90` для четырёх
вариантов. Выбран поэтапный `v0.1.1`; матрица является прозрачным инженерным
средством решения, а не универсальной эмпирической оценкой стратегий.

Контракт будущей реализации сохраняет схему `mlcra_doctor_report_v01` и её
ключи для совместимости. `release_ready` должен означать только отсутствие
известного статического блокера содержимого пакета, а не полномочие владельца
на публикацию; это уточняется новым полем
`release_readiness_scope=packaged_content_only_no_publication_authority`.
При отсутствии такого дефекта `release_ready=true`, `release_blockers=[]`.
Решения John о теге, GitHub Release и PyPI хранятся в репозиторных evidence и
никогда не выводятся из runtime-флага.

SemVer 2.0.0 требует не менять содержимое уже опубликованной версии и относит
`0.y.z` к начальному развитию. Поэтому `0.1.1` используется как локально
определённый совместимый корректирующий идентификатор без заявления
стабильности API. PyPA требует уникального публичного version identifier —
идентификатора версии — и разделяет исходники, сборку и загрузку. GitHub
immutable releases сохраняют tag/assets, а NIST SSDF поддерживает защиту
release-файлов, происхождения и автоматизированных проверок. PyPI остаётся
отложенным; при отдельном будущем разрешении применяется Trusted Publishing с
краткоживущими OIDC tokens — токенами.

### 76.3. Фазы, доказательная граница и риск

После текущего контракта зарегистрированы, но не авторизованы:

1. `ST08_15A_v0_1_1_corrective_candidate_implementation` — узкая реализация,
   тесты и локальные воспроизводимые активы;
2. `ST08_15B_v0_1_1_final_candidate_reaudit_and_security_preflight` — проверка
   совместимости, требований, безопасности, воспроизводимости и hosted CI;
3. `ST08_15C_v0_1_1_GitHub_execution` — отдельное неизменяемое выполнение
   GitHub Release;
4. `ST08_15D_optional_PyPI_trusted_publication` — необязательный отдельный
   канал после принятого `v0.1.1`.

Текущий блок не выполняет ни одну из этих фаз. SHA-256 связывает байты, но не
доказывает корректность или научную валидность. Защищённые 18 научных
артефактов не изменяются; обучения и нового научного вывода нет. Checkpoint ZIP
пропущен: изменение обратимо, а защищённый удалённый Git и successor manifest
обеспечивают восстановление.

## 77. Задачи John

1. Создать commit из точного change set — набора изменений — ST08_15,
   отправить ветвь, открыть pull request в `main` и дождаться обязательного
   `assurance`, включающего `final-evidence`.
2. После регистрации успешного hosted run принять либо вернуть только ST08_15.
3. При принятии только John может присвоить `ACCEPTED_BY_JOHN` и `TASK_CLOSED`.
4. Отдельно решить, авторизовать ли ST08_15A; текущая задача не разрешает
   реализацию коррекции или любую публикацию.
